...
port = 5004  # Set port for current program
ip = '192.168.0.127'   # Set your IP
initial_peer = (ip, 5003)  # Update this to the address of the previous node
node_id = 'Node4'
...
class Block:
    def __init__(self, index: int, timestamp: Optional[float], transactions: List[Transaction], previous_hash: str):
        self.index = index
...
class Blockchain:
    def __init__(self, node_id, initial_peer):
        self.node_id = node_id
        self.peer_list = [initial_peer]
        self.chain = self.get_chain_from_peer()  # Fetch the initial chain from the peer
        self.transactions = []
    ...
    def send_prepare_message(self):
        ...
        self.state = "COMMITED"
        self.wait_for_prepare_messages(valid_peer, 0.5)

    def wait_for_prepare_messages(self, valid_peer, interval=0.5):
        # bool for number of correct messages
        condition = False
        while True:
            if len(self.peer_list) >= valid_peer:
                condition = True
                break
            time.sleep(interval)
        if condition:
    ...
    def send_commit_message(self):
        ...
        for peer in self.peer_list:
            try:
                ...
                if response.status_code == 200:
                    print(f"Commit message sent to {peer}")
                else:
                    print(f"Failed to send prepare message to {peer} with status code {response.status_code}")
            except Exception as e:
                print(f"Error sending prepare message to {peer}: {e}")
        self.state = "INSERTED"
        self.wait_for_commit_messages(valid_peer, 0.5)

    def wait_for_commit_messages(self, valid_peer=1, interval=0.5):
        # bool for number of correct messages
        condition = False
        while True:
            if len(self.peer_list) >= valid_peer:
    ...
    def get_chain_from_peer(self):
        try:
            response = requests.get(f'http://{self.peer_list[0][0]}:{self.peer_list[0][1]}/display_chain')

            if response.status_code == 200:
                data = response.json()

                # Extract chain data from JSON
                chain_data = data.get('chain', [])
                peers_data = data.get('peers', [])

                # Convert the received data into Block objects
                chain = []
                for block_data in chain_data:
                    transactions_data = block_data.get("Transactions", [])
                    transactions = [Transaction(node_id=txn["node_id"], generic_data=txn["generic_data"]) for txn in transactions_data]

                    block = Block(
                        index=block_data.get("Index"),
                        timestamp=block_data.get("Timestamp"),
                        transactions=transactions,
                        previous_hash=block_data.get("Previous Hash")
                    )
                    chain.append(block)

                # Add peers to peer_list
                ...
                print(f"Failed to fetch chain from peer. Status code: {response.status_code}")
                return []
    ...
    def add_block(self, transactions: List[Transaction]):
        ...
        new_block = Block(index=len(self.chain), timestamp=time(), transactions=transactions, previous_hash=previous_block.current_hash)
        self.chain.append(new_block)


    def add_peer(self, peer):
        # Check if the peer is not already in the list
        if peer not in self.peer_list:
            # Add a new peer to the peer_list
            self.peer_list.append(peer)
            print(f"Added peer: {peer}")
        else:
            print(f"Peer {peer} already in the list")
    ...
    def display_chain(self):
        ...
        # Display connected peers with IP addresses and ports
    ...
    def send_heartbeat(self):
        while True:
            ...
            sleep(5)  # Send heartbeat every 10 seconds
...
def add_node_globally(ip, port):
    data = {"new_node": {"ip": ip, "port": port}}

    addresses = [f"http://{ip}:5001/register_node", f"http://{ip}:5002/register_node",  f"http://{ip}:5003/register_node"]
...
def display_chain():
    ...
    blockchain.save_chain_to_file('data_blockchain/blockchain_data4.json')
...
def receive_prepare_message():
    try:
        ...
        print(data)
        print(blockchain.prepare_messages)
