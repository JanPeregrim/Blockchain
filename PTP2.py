...
port = 5002
ip = '192.168.0.127'
initial_peer = (ip, 5001)  # Update this to the address of the first node
node_id = 'Node2'
...
class Blockchain:
    ...
    def wait_for_prepare_messages(self, valid_peer, interval=0.5):
        ...
        condition = False
        while True:
            print(self.prepare_messages)
            print(len(self.prepare_messages))
    ...
    def wait_for_commit_messages(self, valid_peer, interval=0.5):
        # bool for number of correct messages
        condition = False
        while True:
            if len(self.peer_list) >= valid_peer:
...
def add_node_globally(ip, port):
    ...
    addresses = [f"http://{ip}:5001/register_node"]
...
def display_chain():
    ...
    blockchain.save_chain_to_file('data_blockchain/blockchain_data2.json')
