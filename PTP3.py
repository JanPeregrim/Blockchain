...
port = 5003
ip = '192.168.0.127'
initial_peer = (ip, 5002)  # Update this to the address of the first node
node_id = 'Node3'
        ...
        for peer in self.peer_list:
            try:
                ...
                if response.status_code == 200:
                    print(f"Prepare message sent to {peer}/prepare")
                else:
...
def add_node_globally(ip, port):
    ...
    addresses = [f"http://{ip}:5001/register_node", f"http://{ip}:5002/register_node"]
...
def display_chain():
    ...
    blockchain.save_chain_to_file('data_blockchain/blockchain_data3.json')
